    ...
    config_dict = {# Monta configuração do coletor
        ...
        "processors": {
            "tail_sampling": {
                "decision_wait": "30s",
                "num_traces": 15000,
                "expected_new_traces_per_sec": 750,
...
def reward_function(entropy, traces, alpha=1.0, beta=1.0, C = 12000, lambd=3.0):
...
if __name__ == "__main__":
    ...
    # --- NOVO: apenas a política default com 100% de sampling ---
        selected_policies = [{
            "name": "high_price_requests",
            "type": "numeric_attribute",
            "numeric_attribute": {"key": "total", "min_value": 1, "max_value": 100000000}
        }]

        # --- NOVO: hash criada somente a partir do timestamp ---
        timestamp = f"{time.time():.6f}"
        config_hash = hashlib.sha256(timestamp.encode()).hexdigest()[:8]

        # Gera config e segue o fluxo normal
